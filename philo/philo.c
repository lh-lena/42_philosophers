...
int	data_init(t_table *table)
{
	...
	while (++i < table->philo_nbr)
	{
		...
		table->philos[i].last_eat = table->start; // added
...
or equal to the time it takes to a philo to dead from starvation, 
to make everything work a little bit better we are also going to check if in the momet
a philo should dead he's eating, in that case we'll let him survive.
